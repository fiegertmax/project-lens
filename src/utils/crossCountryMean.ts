...
/**
 * Sums each country's co2_including_luc (point.value) over the selected year range
 * to produce a scalar weight per country. Countries with no finite values get weight 0.
...
function computeWeights(
  ...
  for (const country of countries) {
    ...
    for (const p of points) {
      if (p.year >= yearRange[0] && p.year <= yearRange[1] && Number.isFinite(p.value)) {
        total += p.value;
      }
    }
...
export function crossCountryMean(
  ...
  const weights = mode === 'weighted' ? computeWeights(countries, dataset, yearRange) : null;

  return lenses.map((lens) => {
    const values = new Map<string, { left: number | undefined; right: number | undefined }>();
    for (const src of EMISSION_SOURCES) {
      values.set(src.key, {
        left: meanForSourceYear(countries, src.key, lens.startYear, dataset, weights),
        right: meanForSourceYear(countries, src.key, lens.endYear, dataset, weights),
      });
    }
    return { ...lens, values };
  });
